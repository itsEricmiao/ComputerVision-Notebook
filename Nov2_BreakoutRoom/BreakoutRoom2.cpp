...
//  cv::Mat warp_mat = cv::getPerspectiveTransform(srcQuad, dstQuad);
  ...
  cv::Mat warp_mat2 = (cv::Mat_<double>(2,3) << 1,0,96,0,1,96);
  cv::Mat dst_2;
  cv::warpAffine(src_2, dst_2, warp_mat2, src.size(), cv::INTER_LINEAR,
                        cv::BORDER_CONSTANT, cv::Scalar());

  for( int i = 0; i < 4; i++ )
      cv::circle(dst_2, dstQuad[i], 5, cv::Scalar(255, 0, 255), -1, cv::LINE_AA);


  cv::imshow("Scalling", dst);
  cv::imshow("Transformation", dst_2);
  cv::waitKey();
  return 0;
}
